...
func testLayerReuse(t *testing.T, context spec.G, it spec.S) {
	...
	context("an app is rebuilt and tini dependency is unchanged", func() {
		it("reuses a layer from a previous build", func() {
			...
			Expect(firstImage.Buildpacks[0].Layers).To(HaveKey("tini"))
			...
			Expect(secondImage.Buildpacks[0].Layers).To(HaveKey("tini"))
