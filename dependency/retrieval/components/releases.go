...
func (f Fetcher) Get() ([]Release, error) {
	...
	for {
		resp, err := http.Get(fmt.Sprintf("%s/repos/krallin/tini/releases?per_page=100&page=%d", f.api, page))
		if err != nil {
			return nil, err
		}
		...
		for _, release := range releaseResponse {
			...
			var r Release
			r.Version = strings.TrimPrefix(release.TagName, "v")
			...
			r.Files = append(r.Files, ReleaseFile{Name: "source", URL: release.TarballURL})
